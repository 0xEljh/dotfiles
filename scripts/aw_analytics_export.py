...
def aggregate_day_data(day_data: dict) -> dict:
    ...
    for event in window_events:
        ...
        if app in TERMINAL_APPS:
            ...
            dev_tools[coding_app_display_name(app)] += duration

        # Planning apps detection
        if app in PLANNING_APPS:
            planning_apps[app.title()] += duration

        # AI Chat desktop apps detection
        if app in AI_CHAT_APPS:
            display_name = ai_chat_app_display_name(app)
    ...
    for event in web_events:
        ...
        ai_name = match_ai_chat_site(hostname)
        if ai_name:
